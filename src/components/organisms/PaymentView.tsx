...
export default function PaymentView({
  ...
  const [errorRedirectCountdown, setErrorRedirectCountdown] = useState<number>(0);
  const onDashboardRef = useRef(onDashboard);
  onDashboardRef.current = onDashboard;

  useEffect(() => {
    if (!paymentData || paymentData.status !== "PENDING" || !paymentData.expiredAt) {
      setTimeLeft(0);
      return;
    }
  ...
  const isPending = paymentData?.status === "PENDING" || paymentData?.status === "pending";
  const isSuccess = paymentData?.status === "SUCCESS" || paymentData?.status === "settlement";
  const isExpired = paymentData?.status === "EXPIRED" || paymentData?.status === "expire";
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div className="space-y-6">
              <motion.div
                ...
                className="bg-white border border-blue-200 rounded-3xl p-8 shadow-xl"
              >
                <h3 className="mb-6 text-blue-600 font-semibold">Detail Pembayaran</h3>
                <div className="text-center text-gray-500 py-8">Memuat data pembayaran...</div>
              </motion.div>
            </div>
          </div>
        </div>
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-50 to-white flex items-center justify-center p-4">
        ...
        <div className="text-center max-w-md">
          ...
          <p className="text-gray-600 mb-6">{error || "Data pembayaran tidak dapat dimuat"}</p>
          {errorRedirectCountdown > 0 && (
            <p className="text-gray-500 text-sm mb-6">Mengalihkan ke dashboard dalam {errorRedirectCountdown} detik...</p>
          )}
          <motion.button
            ...
            >
              {isExpiredLocally ? (
                <div className="text-center py-8">
                  ...
                  <h3 className="text-xl font-bold text-red-800 mb-2">Pembayaran telah kadaluarsa</h3>
                  <p className="text-red-600 mb-4">Waktu pembayaran telah habis</p>
                  {redirectCountdown > 0 && (
                    <p className="text-gray-500 text-sm">
                      Mengalihkan ke dashboard dalam {redirectCountdown} detik...
                    </p>
                  )}
                </div>
              ) : (
                <>
                  <div className="text-center mb-6">
                    ...
                    <p className="text-gray-600">Gunakan e-wallet atau mobile banking anda</p>
                  </div>
                  ...
                  {isPending && timeLeft > 0 && (
                    <div className="mb-6 p-4 bg-blue-50 rounded-2xl border border-blue-200">
                      <div className="flex items-center justify-center gap-2 text-center">
                        ...
                        <div>
                          <div className="text-gray-600 text-sm">Payment expires in</div>
                          <div className={`${timeLeft < 300 ? "text-red-500" : "text-blue-600"} font-semibold text-lg`}>
                            {formatTime(timeLeft)}
                          </div>
                        </div>
                      </div>
                    ...
                    <div className="aspect-square flex items-center justify-center">
                      {loading ? (
                        ...
                        <div className="text-center text-green-600">
                          ...
                          <p className="text-gray-700 text-lg font-semibold">Pembayaran berhasil!</p>
                          <p className="text-gray-500 text-sm mt-2">Mengalihkan ke dashboard dalam {redirectCountdown} detik...</p>
                        </div>
                        ...
                        <div className="text-center">
                          ...
                          />
                          <p className="text-gray-700 mt-4">Scan QR Code untuk membayar</p>
                        </div>
                      ) : (
                        <div className="text-center text-blue-600">
                  ...
                  <div className="p-6 bg-gradient-to-r from-blue-600 to-blue-500 rounded-2xl text-white shadow-lg space-y-2">
                    <div className="mb-3 font-semibold text-sm text-white">Rincian Pembayaran</div>
                    ...
                    <div className="flex items-center justify-between text-base">
                      ...
                      <span className="font-semibold text-lg">{formatCurrency(paymentData.totalAmount)}</span>
                    </div>
                  </div>
            ...
            <div className="space-y-6">
              ...
              >
                ...
                {isSuccess && (
                  <div className="mt-6 pt-6 border-t border-blue-200">
                    <div className="text-center">
                      ...
                      <h3 className="text-xl font-bold text-green-800 mb-2">{successTitle}</h3>
                      {successMessage && <p className="text-green-700 mb-6">{successMessage}</p>}
                    </div>
                    <div className="space-y-3">
                      {extraSuccessActions}
                      <Button variant="primary" onClick={onDashboard} className="w-full">
                        Ke Dashboard
                      </Button>
                    </div>
                  </div>
                ...
                {isExpired && (
                  <div className="mt-6 pt-6 border-t border-blue-200">
                    <div className="text-center">
                      <div className="text-xl font-bold text-red-800 mb-2">{expiredTitle}</div>
              ...
              {showInstructions && isPending && !isExpiredLocally && (
                <motion.div
                  ...
                  className="bg-white border border-blue-200 rounded-3xl p-8 shadow-xl"
                >
                  <h3 className="mb-6 text-blue-600 font-semibold">Bagaimana cara pembayaran?</h3>
                  <ol className="space-y-4">
                    <li className="flex gap-4">
                      <div className="flex-shrink-0 w-8 h-8 bg-blue-600 text-white rounded-full flex items-center justify-center font-semibold">1</div>
                      <div>
                        <div className="text-gray-900 mb-1 font-medium">Buka aplikasi</div>
                        <div className="text-gray-600 text-sm">Buka aplikasi mobile banking atau e-wallet anda</div>
                      </div>
                    </li>
                    <li className="flex gap-4">
                      <div className="flex-shrink-0 w-8 h-8 bg-blue-600 text-white rounded-full flex items-center justify-center font-semibold">2</div>
                      <div>
                        <div className="text-gray-900 mb-1 font-medium">Scan QR Code</div>
                        <div className="text-gray-600 text-sm">Gunakan fitur scan QRIS yang ada di aplikasi anda</div>
                      </div>
                    </li>
                    <li className="flex gap-4">
                      <div className="flex-shrink-0 w-8 h-8 bg-blue-600 text-white rounded-full flex items-center justify-center font-semibold">3</div>
                      <div>
                        <div className="text-gray-900 mb-1 font-medium">Verifikasi jumlah</div>
                        <div className="text-gray-600 text-sm">Cek kembali apakah jumlah yang dibayarkan sudah sesuai</div>
                      </div>
                    </li>
                    <li className="flex gap-4">
                      ...
                      <div>
                        <div className="text-gray-900 mb-1 font-medium">Selesaikan pembayaran</div>
                        <div className="text-gray-600 text-sm">Konfirmasi dan selesaikan pembayaran anda</div>
                      </div>
                    </li>
                  </ol>
                  <div className="mt-6 p-4 bg-blue-50 rounded-2xl border border-blue-200">
                    <p className="text-gray-600 text-center text-sm">
                      Pembayaran anda akan terkonfirmasi otomatis. Jika belum terkonfirmasi, harap tekan tombol refresh payment.
                    </p>
                  </div>
                </motion.div>
              )}
              ...
              {showWhatsApp && isPending && !isExpiredLocally && (
                <motion.button
                  ...
                  onClick={() => {
                    ...
                    window.open(`https://wa.me/6281385042622?text=${msg}`, "_blank");
                  }}
