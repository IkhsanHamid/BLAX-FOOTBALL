...
import type { AttendanceHistoryData, ScanBookingResponse, ScanLineupData } from "@/types/admin";
...
export default function AttendanceTab() {
  ...
  const [scanResult, setScanResult] = useState<ScanBookingResponse["data"] | null>(null);
  const [scanError, setScanError] = useState("");
  const [scanning, setScanning] = useState(false);
  const [checkingIn, setCheckingIn] = useState(false);
  const [checkinSuccess, setCheckinSuccess] = useState(false);
  const [scanJerseyData, setScanJerseyData] = useState<Record<string, { jerseyNumber: string; jerseySize: string }>>({});
  ...
  const filteredSchedules = useMemo(() => {
    ...
    return schedules.filter(
      (s) => s.name.toLowerCase().includes(q) || formatDate(s.date).toLowerCase().includes(q),
    );
  ...
  const handleCheckin = async () => {
    ...
    try {
      ...
      if (res?.status) {
        ...
        showSuccess("Berhasil", `Checkin berhasil (${res.data?.updatedCount ?? 0} pemain)`);
        if (scanPlayer) {
          const jersey = scanJerseyData[scanPlayer.lineupId] || { jerseyNumber: "", jerseySize: "" };
          const payload: any = {
          ...
          const lineupPayloads = (scanResult.lineups || []).map((lineup: ScanLineupData) => {
            const jersey = scanJerseyData[lineup.lineupId] || { jerseyNumber: "", jerseySize: "" };
            return {
                  ...
                  >
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-xs text-gray-500 flex items-center gap-1 mt-0.5">
                        ...
                        <span className="truncate">{formatDate(s.date)} &bull; {s.time}</span>
                      </div>
                    </div>
      ...
      <div className="flex items-center justify-between flex-wrap gap-2 mb-4">
        ...
        <div className="flex items-center gap-1.5 flex-shrink-0">
          {!history?.penanggungJawab && (
            <>
              <Button variant="primary" size="sm" onClick={() => openScanModal()}>
      ...
      {loadingHistory ? (
        ...
        <div className="space-y-4">
            ...
            <div className="flex items-center justify-between gap-2 mb-2 sm:mb-3">
              <div className="min-w-0">
                <p className="text-xs text-gray-500">
                  {new Date(history.date).toLocaleDateString("id-ID", { day: "numeric", month: "short", year: "numeric" })} • {history.time} WIB
                </p>
                {history.penanggungJawab && (
              ...
              {history.penanggungJawab && (
                <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-slate-100 text-slate-500 flex-shrink-0">Selesai</span>
              )}
            </div>
            <div className="flex gap-3 sm:gap-6">
              {[
                { label: "Hadir", value: history.attendance.hadir, color: "text-green-600 bg-green-50" },
                { label: "Tidak Hadir", value: history.attendance.tidakHadir, color: "text-red-500 bg-red-50" },
                { label: "Total", value: history.attendance.totalPlayers, color: "text-slate-600 bg-slate-50" },
              ].map((s) => (
          ...
          {history.lineup.map((team) => (
            <div key={team.teamName} className="bg-white rounded-xl border border-slate-200 overflow-hidden">
                ...
                <table className="w-full text-xs sm:text-sm">
                  <thead>
                    <tr className="border-b border-slate-100">
                      ...
                      <th className="text-left px-4 py-2 font-medium text-gray-500 w-16">Posisi</th>
                      <th className="text-left px-4 py-2 font-medium text-gray-500 w-20">Jersey</th>
                      <th className="text-left px-4 py-2 font-medium text-gray-500">Hadir</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-50">
                    {team.gk && (
                      <HistoryPlayerRow player={team.gk} onUpdate={() => loadHistory(selectedScheduleId)} />
                    )}
                    {team.players.map((p, i) => (
                      <HistoryPlayerRow key={i} player={p} onUpdate={() => loadHistory(selectedScheduleId)} />
                    ))}
                  </tbody>
                </table>
              </div>
...
function ScanQrModal({
  ...
  setScanJerseyData: (v: Record<string, { jerseyNumber: string; jerseySize: string }>) => void;
  onScan: () => void;
  onCheckin: () => void;
  onClose: () => void;
}) {
      ...
      <DialogContent className="max-w-md">
        ...
        <div className="space-y-4 py-2">
          <p className="text-sm text-gray-500">
            {scanPlayer ? (
              <>Pemain: <span className="font-medium text-gray-800">{scanPlayer.name || "—"}</span></>
            ) : (
              "Scan QR code pemain"
            )}
          </p>
          ...
          {!showManual ? (
            <div>
              ...
              <button
                onClick={() => { if (scannerRef.current && scannerStartedRef.current) { scannerRef.current.stop().then(() => { scannerStartedRef.current = false; scannerRef.current = null; }).catch(() => {}); } else { scannerRef.current = null; } setShowManual(true); }}
            ...
            <div>
              <div className="flex flex-col sm:flex-row gap-3" onKeyDown={(e) => e.key === "Enter" && onScan()}>
                <div className="relative flex-1">
                ...
                <Button variant="primary" onClick={onScan} disabled={scanning || !scanBookId.trim()}>
          ...
          {scanResult && (
            <div className="space-y-3">
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  <h3 className="font-semibold text-gray-900">{scanResult.customerName}</h3>
              ...
              {checkinSuccess ? (
                <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-4 flex items-start gap-3">
                  <CheckCircle className="w-5 h-5 text-emerald-600 flex-shrink-0 mt-0.5" />
                  <div>
                    <p className="text-sm font-semibold text-emerald-800">Checkin berhasil!</p>
                    <p className="text-xs text-emerald-600 mt-1">Semua pemain telah dikonfirmasi hadir.</p>
                  </div>
                </div>
              ) : (
                <>
                  <div className="grid grid-cols-2 gap-3 text-sm">
                    <div className="bg-gray-50 rounded-lg px-3 py-2">
                      <p className="text-[10px] text-gray-400 mb-0.5">Schedule</p>
                      <p className="font-medium text-gray-800">{scanResult.scheduleName}</p>
                    </div>
                    <div className="bg-gray-50 rounded-lg px-3 py-2">
                      <p className="text-[10px] text-gray-400 mb-0.5">Tanggal</p>
                      <p className="font-medium text-gray-800">{new Date(scanResult.date).toLocaleDateString("id-ID")}</p>
                    </div>
                    <div className="bg-gray-50 rounded-lg px-3 py-2">
                      <p className="text-[10px] text-gray-400 mb-0.5">Waktu</p>
                      <p className="font-medium text-gray-800">{scanResult.time}</p>
                    </div>
                    <div className="bg-gray-50 rounded-lg px-3 py-2">
                      <p className="text-[10px] text-gray-400 mb-0.5">Venue</p>
                      <p className="font-medium text-gray-800">{scanResult.venue}</p>
                    </div>
                    <div className="bg-gray-50 rounded-lg px-3 py-2 col-span-2">
                      <p className="text-[10px] text-gray-400 mb-0.5">Tim</p>
                      <p className="font-medium text-gray-800">{scanResult.teamName}</p>
                    </div>
                  </div>
                      ...
                      <div className="max-h-60 overflow-y-auto space-y-1.5">
                        {scanResult.lineups.map((lineup: ScanLineupData) => {
                          ...
                          return (
                            ...
                            >
                              <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded w-12 text-center flex-shrink-0 ${
                                lineup.position === "GK" ? "bg-amber-100 text-amber-700" : "bg-blue-50 text-blue-600"
                              }`}>
                                {lineup.position}
                              </span>
                              <div className="flex items-center gap-1.5 flex-1 min-w-0">
                                <input
                                  ...
                                  onChange={(e) => setScanJerseyData({
                                    ...scanJerseyData,
                                    [lineup.lineupId]: { ...jersey, jerseyNumber: e.target.value },
                                  })}
                                ...
                                <input
                                  ...
                                  onChange={(e) => setScanJerseyData({
                                    ...scanJerseyData,
                                    [lineup.lineupId]: { ...jersey, jerseySize: e.target.value },
                                  })}
                                  className="w-14 px-1.5 py-1 text-xs border border-gray-300 rounded focus:ring-1 focus:ring-sky-500"
                                />
                              </div>
...
function HistoryPlayerRow({ player, onUpdate }: { player: any; onUpdate: () => void }) {
      ...
      <tr>
        ...
        <td className="px-4 py-2">
          <div className="flex items-center gap-1 flex-wrap">
            ...
            <button onClick={handleSave} disabled={saving} className="p-1 text-green-600 hover:text-green-800" title="Simpan">
              <Save className="w-3.5 h-3.5" />
            </button>
            <button onClick={() => setEditing(false)} disabled={saving} className="p-1 text-gray-400 hover:text-gray-600" title="Batal">
  ...
  return (
    <tr className="hover:bg-slate-50/50">
      <td className="px-4 py-2.5 text-sm font-medium text-gray-800 truncate max-w-[120px]">{player.name || "—"}</td>
